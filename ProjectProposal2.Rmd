...
Population: This data set is specific to sports cars from various manufacturers. Therefore, it would likely not be wise to expand this model to all cars or car manufacturers outside of the manufacturers included in the data set. The data set from Kaggle is a sample from sports cars in America, so we hope to be able to use this model as a way to predict the price of sports cars in the US so that hopeful owners can get the right car for the right price.
...
favstats(~price_in_usd, data=SportsCars)

gf_boxplot(~year, data=SportsCars) #this needs to be smaller?
```
...
```{r warning=FALSE}
tally(~car_make, data = SportsCars) #we figured this would be the better than a pie chart, likely to throw out makes that are singular 
tally(~car_model, data= SportsCars)

m5<-gf_bar (~car_make, data= SportsCars)
m6<-gf_bar (~car_model, data= SportsCars)
grid.arrange(m1, m2, m3, m4, m5, m6, ncol=3, heights=c(4, 4))
